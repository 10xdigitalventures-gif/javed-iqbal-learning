...
export default function AdminPackages() {
  ...
  async function load() {
    try {
      const [pkgs, people] = await Promise.all([
        api<Package[]>("/tenant-admin/packages"),
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      if (editingId) {
        await api(`/tenant-admin/packages/${editingId}`, { method: "PATCH", body });
      } else {
        await api("/tenant-admin/packages", { method: "POST", body });
      }
      setShow(false);
      setEditingId(null);
      setForm(EMPTY);
  ...
  async function remove(id: string) {
    await api(`/tenant-admin/packages/${id}`, { method: "DELETE" });
    load();
  }
