...
function ChapterManager({ bookId }: { bookId: string }) {
  ...
  async function load() {
    setLoading(true);
    try {
      const d = await api<AdminChapter[]>(
        `/tenant-admin/books/${bookId}/chapters/admin`,
      );
      setChapters(
  ...
  async function addChapter() {
    setError(null);
    try {
      await api(`/tenant-admin/books/${bookId}/chapters`, {
        method: "POST",
        body: {
  ...
  async function removeChapter(c: AdminChapter) {
    ...
    try {
      await api(`/tenant-admin/books/chapters/${c.id}`, { method: "DELETE" });
      await load();
    } catch (e: any) {
      setError(e.message);
    }
  ...
  async function move(idx: number, dir: -1 | 1) {
    ...
    try {
      await api(`/tenant-admin/books/${bookId}/chapters/reorder`, {
        method: "PUT",
  ...
  async function importPdf(file: File, replace: boolean, ocr = false) {
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/tenant-admin/books/${bookId}/import-pdf?replace=${replace}&ocr=${ocr}`,
        {
          method: "POST",
          credentials: "include",
  ...
  async function pollImportJob(
    ...
    for (let i = 0; i < 1200; i++) {
      ...
      try {
        const res = await fetch(
          `${API_URL}/tenant-admin/books/${bookId}/import-jobs/${jobId}`,
          {
            credentials: "include",
            headers: token ? { Authorization: `Bearer ${token}` } : {},
          },
...
export default function EbooksAdminPage() {
  ...
  function loadBooks() {
    ...
    return api<Paged<Book>>(`/tenant-admin/books/paged${query}`)
      .then((d) => {
  ...
  async function saveBook(e: React.FormEvent) {
    ...
    if (editing) {
      await api(`/tenant-admin/books/${editing.id}`, { method: "PATCH", body });
    } else {
      const created = await api<Book>("/tenant-admin/books", {
        method: "POST",
        body,
      });
  ...
  async function removeBook(id: string) {
    ...
    await api(`/tenant-admin/books/${id}`, { method: "DELETE" });
    await loadBooks();
  }
      ...
      <Card className="mt-6">
        <div className="mb-3">
          ...
          <p className="text-sm text-slate-500">
            Categories are managed at the platform level. Tenant admins can
            assign them to books but cannot create or rename global categories
            here.
          </p>
        </div>
