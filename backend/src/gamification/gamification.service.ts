...
export type BadgeMetric = "lessons" | "courses" | "reviews" | "streak" | "points";
...
export class GamificationService {
  ...
  static readonly BADGES: BadgeDef[] = [
    { key: "first_lesson", name: "First Steps", description: "Complete your first lesson", icon: "\uD83C\uDFAF", metric: "lessons", threshold: 1 },
    { key: "ten_lessons", name: "Getting Serious", description: "Complete 10 lessons", icon: "\uD83D\uDCDA", metric: "lessons", threshold: 10 },
    { key: "fifty_lessons", name: "Scholar", description: "Complete 50 lessons", icon: "\uD83C\uDFC5", metric: "lessons", threshold: 50 },
    { key: "first_course", name: "Course Crusher", description: "Finish your first course", icon: "\uD83C\uDF93", metric: "courses", threshold: 1 },
    { key: "five_courses", name: "Lifelong Learner", description: "Finish 5 courses", icon: "\uD83D\uDC51", metric: "courses", threshold: 5 },
    { key: "first_review", name: "Critic", description: "Write your first course review", icon: "\u270D\uFE0F", metric: "reviews", threshold: 1 },
    { key: "streak_3", name: "Warming Up", description: "Keep a 3-day learning streak", icon: "\uD83D\uDD25", metric: "streak", threshold: 3 },
    { key: "streak_7", name: "On Fire", description: "Keep a 7-day learning streak", icon: "\uD83D\uDD25", metric: "streak", threshold: 7 },
    { key: "streak_30", name: "Unstoppable", description: "Keep a 30-day learning streak", icon: "\u26A1", metric: "streak", threshold: 30 },
    { key: "points_500", name: "High Achiever", description: "Earn 500 points", icon: "\u2B50", metric: "points", threshold: 500 },
    { key: "points_1000", name: "Legend", description: "Earn 1000 points", icon: "\uD83C\uDF1F", metric: "points", threshold: 1000 },
  ];

  // ---- internal helpers ----

  private async getOrCreateStat(userId: string) {
    const existing = await this.prisma.userGameStat.findUnique({ where: { userId } });
  ...
  private async metricsFor(
    userId: string,
    stat: { currentStreak: number; longestStreak: number },
  ): Promise<Metrics> {
  ...
  private async syncBadges(userId: string, metrics: Metrics) {
    ...
    const toAward = GamificationService.BADGES.filter(
      (def) => !earnedKeys.has(def.key) && this.metricValue(metrics, def) >= def.threshold,
    );
    for (const def of toAward) {
      try {
        await this.prisma.userBadge.create({ data: { userId, badgeKey: def.key } });
      } catch {
  ...
  async getProfile(userId: string) {
    ...
    const earned = await this.prisma.userBadge.findMany({ where: { userId } });
    const earnedMap = new Map(earned.map((b) => [b.badgeKey, b.earnedAt] as const));
    ...
    return {
      ...
      stats: { lessons: metrics.lessons, courses: metrics.courses, reviews: metrics.reviews },
      badges,
  ...
  async checkIn(userId: string, day?: string) {
    ...
    } else if (stat.lastActiveDay && this.isYesterday(stat.lastActiveDay, today)) {
      currentStreak += 1;
    } else {
      currentStreak = 1;
    }
