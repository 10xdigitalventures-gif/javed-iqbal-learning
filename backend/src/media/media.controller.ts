...
export class MediaController {
  ...
  private cloudEnabled(): boolean {
    ...
    if (p === "supabase") return !!(process.env.SUPABASE_URL && process.env.SUPABASE_KEY);
    if (p === "bunny") return !!process.env.BUNNY_STORAGE_KEY;
    if (p === "r2" || p === "s3") return !!(process.env.CLOUDFLARE_R2_KEY || process.env.S3_ACCESS_KEY_ID);
    return false;
  }
  ...
  async upload(@CurrentUser() user: AuthUser, @UploadedFile() file?: Express.Multer.File) {
    ...
    let uploadPath = file.path, uploadName = file.filename;
    let storedMime = file.mimetype, storedSize = file.size;
    if (file.mimetype.startsWith("video/")) {
      const out = await this.media.transcodeToMp4(file.path);
      if (out) {
        try { if (out.path !== file.path) unlinkSync(file.path); } catch {}
        uploadPath = out.path; uploadName = out.filename;
        storedMime = "video/mp4"; storedSize = out.size;
      }
    }
    if (file.mimetype.startsWith("audio/")) {
      ...
      if (out) {
        try { if (out.path !== file.path) unlinkSync(file.path); } catch {}
        uploadPath = out.path; uploadName = out.filename;
        storedMime = "audio/mp4"; storedSize = out.size;
      }
    }
    if (this.cloudEnabled()) {
      ...
      const asset = await this.media.recordAsset({ ownerId: user.userId, key, mimeType: storedMime, filename: file.originalname, size: storedSize, durationSec, storage: "cloud" });
      return { assetId: asset.id, key, url, filename: uploadName, mimetype: storedMime, size: storedSize, durationSec, storage: "cloud" };
    }
    const asset = await this.media.recordAsset({ ownerId: user.userId, key: uploadName, mimeType: storedMime, filename: file.originalname, size: storedSize, durationSec, storage: "local" });
    return { assetId: asset.id, key: uploadName, url: this.storage.signedUrl(uploadName), filename: uploadName, mimetype: storedMime, size: storedSize, durationSec, storage: "local" };
  }
  ...
  async library(@CurrentUser() user: AuthUser, @Query("type") type?: string) {
    ...
    return Promise.all(items.map(async (it) => {
      ...
      try { url = await this.urlForKey(it.key); } catch (e: any) { urlError = e?.message || "Could not generate URL"; }
  ...
  async share(@Param("id") id: string) {
    ...
    return { url: await this.urlForKey(asset.key, SHARE_TTL_SEC), type: asset.type, filename: asset.filename, mimeType: asset.mimeType, durationSec: asset.durationSec };
  }
  ...
  async remove(@CurrentUser() user: AuthUser, @Param("id") id: string) {
    const asset = await this.media.getAsset(id);
    if (!isAdmin(user) && asset.ownerId !== user.userId) throw new ForbiddenException("Not allowed to delete this item");
    try { if (this.cloudEnabled()) await this.cloud.deleteFile(asset.key); } catch {}
  ...
  @Get("file/:key")
  file(@Param("key") key: string, @Query("exp") exp: string, @Query("sig") sig: string, @Res() res: Response) {
    if (this.cloudEnabled() || this.storage.isS3()) throw new NotFoundException("Local delivery disabled when using cloud storage");
    if (!this.storage.verifyLocalSignature(key, Number(exp), sig)) throw new BadRequestException("Invalid or expired link");
  ...
  async drmLicense(
    ...
    if (!result.valid) throw new UnauthorizedException("Invalid or expired DRM token");
    ...
    const upstream = await fetch(licenseServer, { method: "POST", headers, body });
    const licBytes = await upstream.buffer();
    res.status(upstream.status).set("Content-Type", "application/octet-stream").send(licBytes);
  }
}
